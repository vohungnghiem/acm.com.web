...
@section('title', 'Admincp | Hình ảnh')
@push('main') Trang chủ @endpush
@push('item') Hình ảnh @endpush
                        ...
                        <div class="card card-primary card-outline">
                            <div class="card-header">
                                ...
                                <div class="card-tools">
                                    <a class="btn btn-sm btn-primary" href="admincp/hinhanhs/create"><i class="fas fa-plus"> </i> Tạo hình ảnh</a>
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    <thead>
                                        ...
                                        @foreach ($hinhanh as $key => $item)
                                            <tr>
                                                ...
                                                <td>
                                                    {{-- @php
                                                        $image = json_decode($item->hinhanh);
                                                    @endphp --}}
                                                    @foreach (json_decode($item->hinhanh) as $im)
                                                        <img data-toggle="tooltip" title="<img src='{{$im}}'/>" src="{{$im}}" data-html="true" alt="image" style="height: 20px">
                                                    @endforeach
                                                </td>
                                                <td>
                                                    @if ($item->stt == 1)
                                                ...
                                                <td>
                                                    <a href="admincp/hinhanhs/edit/{{$item->id}}" class="btn btn-xs btn-primary" data-toggle="tooltip" title="@lang('admin.update_info')">
...
@push('scripts')
    ...
    <script>
        status('admincp/hinhanhs');
        destroy('admincp/hinhanhs');
    </script>
@endpush
